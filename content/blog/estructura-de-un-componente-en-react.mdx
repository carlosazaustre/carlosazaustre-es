...
seoTitle: Estructura de componentes en React
---
...
En el método `render` creamos una variable `options` con el valor que tenga `this.props.options`. Éste _options_ dentro de _props_ es el mismo atributo _options_ que tiene el componente `<Menu />`y es a través de `props` como le pasamos el valor desde el _padre_ al componente _hijo_.
...
Si tomamos el código anterior y en el componente `<App />`guardamos en su estado las opciones de menu, el código de _App_ sería así:
